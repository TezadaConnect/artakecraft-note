      ...
      <div className="">
        {isShow && (
          <div className="relative w-full flex h-40 mb-2">
            <Image src={projectInfo?.image?.url as string} width={120} height={100} alt="book-cover" className="object-cover rounded bg-slate-800" />
          </div>
        )}
        <h1 className="truncate font-semibold">{projectInfo?.title}</h1>
        <p className="text-[10px] text-slate-400 italic">Genre: {projectInfo?.genre?.join(', ')}</p>
        {isShow && <p className="text-[12px] text-slate-500 italic mt-2">{projectInfo?.synopsis}</p>}
      </div>
